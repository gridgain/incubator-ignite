...
public class ReliabilityTest {
    ...
    public void testFailover() throws Exception {
        try (LocalIgniteCluster cluster = LocalIgniteCluster.start(2);
             IgniteClient client = IgniteClient.start(new IgniteClientConfiguration(cluster.clientAddresses()))
        ) {
            final Random rnd = new Random();

            final CacheClient<Integer, String> cache = client.getOrCreateCache(
                new CacheClientConfiguration("testFailover").setCacheMode(CacheMode.REPLICATED)
            );
