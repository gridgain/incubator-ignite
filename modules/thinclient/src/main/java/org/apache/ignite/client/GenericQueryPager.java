...
abstract class GenericQueryPager<T> implements QueryPager<T> {
    ...
    /** {@inheritDoc} */
    @Override public Collection<T> next() throws IgniteClientException {
        if (!hasNext)
    ...
    private Collection<T> queryPage() throws IgniteClientException {
        try {
            return ch.service(pageQryOp, req -> req.writeLong(cursorId), this::readResult);
        }
        catch (IgniteServerError ex) {
            if (ex.getEvent() != SystemEvent.RESOURCE_DOES_NOT_EXIST)
                throw ex;
        }
        catch (IgniteUnavailableException ignored) {
        }
