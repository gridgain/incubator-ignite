 ...
 *
 */
public class RobinHoodBackwardShiftHashMapV2 implements LoadedPagesMap {
    /** Load factor. */
    ...
    public static long requiredMemory(long elementsCnt) {
        ...
        long buckets = (long)(elementsCnt * loadFactor);

        int pow = 0;

        while ((1L << pow) < buckets) pow++;

        return requiredMemoryByBuckets(1L << pow);
    }
    ...
    static long requiredMemoryByBuckets(long numBuckets) {
        return numBuckets * BYTES_PER_CELL;
    }
     ...
     */
    public RobinHoodBackwardShiftHashMapV2(long baseAddr, long size) {
        this.numBuckets = (int)((size) / BYTES_PER_CELL);
        this.baseAddr = baseAddr;

        int pow = 0;

        while ((1L << pow) < numBuckets) pow++;

        assert (1L << pow) == numBuckets;

        this.powNumBuckets = pow;
    ...
    private long entryBase(int idx) {
        ...
        return baseAddr + ((long)idx << POW_BYTES_PER_CELL);
    }
    ...
    @Override public long get(int grpId, long pageId, int reqVer, long absent, long outdated) {
        ...
        int idxInit = U.safeAbs(FullPageId.hashCode(grpId, pageId)) & (numBuckets - 1);

        for (int i = 0; i < numBuckets; i++) {
            int idxCurr = (idxInit + i) & (numBuckets - 1);
