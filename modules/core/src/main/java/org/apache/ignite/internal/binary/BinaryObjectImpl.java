...
public final class BinaryObjectImpl extends BinaryObjectExImpl implements Externalizable, KeyCacheObject {
    ...
    @Override public int putValue(long addr) throws IgniteCheckedException {
        ...
        if (detached()) {
            byte[] compressed = tryCompress(arr);
        ...
        return CacheObjectAdapter.putValue(addr, type, arr, start);
    }
    ...
    @Override public boolean putValue(final ByteBuffer buf, int off, int len) throws IgniteCheckedException {
        ...
        if (detached()) {
            ...
            if (compressed != null) {
                type = CacheObject.TYPE_BINARY_COMPRESSED;
                arr = compressed;
                len = Math.min(len, CacheObjectAdapter.objectPutSize(compressed.length));
            }
        }
    ...
    @Override public int valueBytesLength(CacheObjectContext ctx) throws IgniteCheckedException {
        if (detached()) {
            ...
            if (compressed != null)
                return CacheObjectAdapter.objectPutSize(compressed.length);
        }

        return CacheObjectAdapter.objectPutSize(length());
    }

    private byte[] tryCompress(byte[] arr) {
        if (compressedArr == null) {
            byte[] tryCompressed = ctx.compress(arr);
            compressedArr = (tryCompressed == null) ? UNCOMPRESSED : tryCompressed;
        }

        return compressedArr.length == 0 ? null : compressedArr;
    }
