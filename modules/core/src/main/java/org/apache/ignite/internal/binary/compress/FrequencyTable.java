 ...
 * @see CodeTree
 */
public final class FrequencyTable<T> {
	 ...
	 */
	public FrequencyTable(int[] freqs, T[] syms) {
		Objects.requireNonNull(freqs);
		if (freqs.length < 2 || freqs.length != syms.length)
			throw new IllegalArgumentException("At least 2 symbols needed, same number of freqs and syms");
	 ...
	 */
	public CodeTree buildCodeTree(Map<T, List<Integer>> backingMap) {
		...
		return new CodeTree((InternalNode)pqueue.remove().node, frequencies.length, backingMap);
	}


	// Helper structure for buildCodeTree()
	private static class NodeWithFrequency<T> implements Comparable<NodeWithFrequency> {
		...
		public NodeWithFrequency(T nd, int lowSym, long freq) {
			node = nd;
			lowestSymbol = lowSym;
			frequency = freq;
		}
