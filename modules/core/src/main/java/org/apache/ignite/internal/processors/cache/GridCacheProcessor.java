...
public class GridCacheProcessor extends GridProcessorAdapter {
    ...
    public Collection<DynamicCacheDescriptor> startReceivedCaches(UUID nodeId, AffinityTopologyVersion exchTopVer)
        ...
        for (DynamicCacheDescriptor desc : started) {
            ...
            if (CU.affinityNodeForDAT(ctx.discovery().localNode(), filter)) { //TODO check baseline for persistent caches
                prepareCacheStart(
                    desc.cacheConfiguration(),
                    desc,
                    null,
    ...
    void prepareCacheStart(
        ...
        else if (CU.affinityNodeForDAT(ctx.discovery().localNode(), desc.groupDescriptor().config().getNodeFilter()))
            affNode = true;
        else {
            affNode = false;
        ...
        if (ccfg != null) {
            ...
            if (desc != null || MetaStorage.METASTORAGE_CACHE_NAME.equals(cacheName)) {
                if (failIfExists) {
                    ...
                    if (nearCfg != null) {
                        if (CU.affinityNodeForDAT(ctx.discovery().localNode(), descCfg.getNodeFilter())) {
                    ...
                    else if (!CU.affinityNodeForDAT(ctx.discovery().localNode(), descCfg.getNodeFilter()))
                        req.clientStartOnly(true);
