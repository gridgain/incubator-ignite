...
public abstract class BPlusTree<L, T extends L> extends DataStructure implements IgniteTree<L, T> {
     ...
     * @throws IgniteCheckedException If failed.
     */
    public void visit(L lower, Object x, TreeVisitorClosure<L, T> c) throws IgniteCheckedException {
        checkDestroyed();

        try {
            new TreeVisitor(lower, x, c).visit();
        }
        catch (IgniteCheckedException e) {
            throw new IgniteCheckedException("Runtime failure on bounds: [lower=" + lower + "]", e);
        }
        catch (RuntimeException e) {
            throw new IgniteException("Runtime failure on bounds: [lower=" + lower + "]", e);
        }
        catch (AssertionError e) {
            throw new AssertionError("Assertion error on bounds: [lower=" + lower + "]", e);
        }
        finally {
            checkDestroyed();
        }
    ...
    private final class TreeVisitor extends Get {
         ...
         * @param lower Lower bound.
         */
        TreeVisitor(L lower, Object x, TreeVisitorClosure<L, T> p) {
            super(lower, false);
        ...
        private void visit(
            ...
            for (int i = startIdx; i < io.getCount(pageAddr); i++) {
         ...
         * @throws IgniteCheckedException If failed.
         */
        private boolean visitPages() throws IgniteCheckedException {
            if (nextPageId == 0)
                return true;
            ...
            try {
                ...
                try {
                    do {
                        ...
                        visit(pageAddr, io, startIdx, io.getCount(pageAddr));
                        ...
                        long pa = pageAddr;
                        pageAddr = 0; // Set to zero to avoid double unlocking in finalizer.
                        readUnlock(pageId, page, pa);

                        long p = page;
                        page = 0; // Set to zero to avoid double release in finalizer.
