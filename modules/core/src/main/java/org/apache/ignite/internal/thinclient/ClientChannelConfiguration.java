...
final class ClientChannelConfiguration {
    ...
    ClientChannelConfiguration(IgniteClientConfiguration cfg) {
        this.addr = cfg.getParsedAddresses().iterator().next(); // configuration always has at least one address
     ...
     */
    public ClientChannelConfiguration setAddresses(InetSocketAddress newVal) {
        addr = newVal;
