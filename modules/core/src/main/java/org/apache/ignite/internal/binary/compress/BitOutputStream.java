...
public final class BitOutputStream implements AutoCloseable {
	 ...
	 * @throws IOException if an I/O exception occurred
	 */
	public void write(int numBits, int b) throws IOException {
		while (numBits > 0) {
	...
	/* Bit order is incorrect
	public void write(int numBits, int input) throws IOException {

		while (numBits > 0) {
			if (numBitsFilled == 0 && numBits > 8) {
				output.write((byte) input & 0xff);
				input >>= 8;
				numBits -= 8;
			}
			else if (numBitsFilled == 0) {
				currentByte = (byte) input & 0xff;
				numBitsFilled = numBits;
				numBits = 0;
			}
			else {
				int sh = Math.min(numBits, 8 - numBitsFilled);
				currentByte = (currentByte << sh) | (input & ((1 << sh) - 1));
				numBitsFilled += sh;
				input >>= sh;
				numBits -= sh;
