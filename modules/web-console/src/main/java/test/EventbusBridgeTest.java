 ...
 */
public class EventbusBridgeTest extends AbstractVerticle {
    /**
     *
     */
    ...
    protected HttpClientOptions getHttpClientOptions() {
        return new HttpClientOptions().setDefaultPort(3000);
    }
    ...
    public void start() throws Exception {
        ...
        client.websocket(websocketURI, ws -> {
            ws.closeHandler(v -> System.out.println("Closed!"));

            JsonObject msg = new JsonObject().put("type", "send").put("address", addr).put("body", "foobar");
            ws.writeTextMessage(msg.toString()); //Frame(io.vertx.core.http.WebSocketFrame.textFrame(msg.encode(), true));

            ws.writeFrame(WebSocketFrame.pingFrame(Buffer.buffer().appendString("PING")));

            ws.writeTextMessage("PING");
//      vertx.setPeriodic(1000, tid -> ws.writeTextMessage("ping"));
//
...
//      });
    ...
    public static void main(String... args) {
        Vertx.vertx().deployVerticle(new EventbusBridgeTest());
    }
