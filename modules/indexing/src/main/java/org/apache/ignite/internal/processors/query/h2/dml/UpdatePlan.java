...
public final class UpdatePlan {
    ...
    public UpdateSourceIterator<?> iteratorForTransaction(IgniteH2Indexing idx, QueryCursor<List<?>> cur) {
        switch (mode) {
            case MERGE:
                return new InsertIterator(idx, cur, this, enlistOperation(mode));
            case INSERT:
                return new InsertIterator(idx, cur, this, enlistOperation(mode));
            case UPDATE:
                return new UpdateIterator(idx, cur, this, enlistOperation(mode));
            case DELETE:
                return new DeleteIterator(idx, cur, this, enlistOperation(mode));
