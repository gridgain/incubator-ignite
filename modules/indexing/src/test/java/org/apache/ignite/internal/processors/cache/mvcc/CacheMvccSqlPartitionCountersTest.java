...
/**
 * Sql partition counters test.
 */
@SuppressWarnings("unchecked")
public class CacheMvccSqlPartitionCountersTest  extends CacheMvccAbstractTest {
    /** {@inheritDoc} */
                ...
                try (Transaction tx = ignite.transactions().txStart(TransactionConcurrency.PESSIMISTIC, TransactionIsolation.REPEATABLE_READ)) {
                    for (int i = 0; i < SIZE; i++)
                        if (ThreadLocalRandom.current().nextBoolean())
                            cache.query(new SqlFieldsQuery("MERGE INTO Integer (_key, _val) VALUES (" + i + ", " + i * 2 + ")")).getAll();
                        else
                            cache.query(new SqlFieldsQuery("DELETE FROM Integer WHERE _key=" + i)).getAll();
