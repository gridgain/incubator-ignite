        ...
        try (Ignite ignite = Ignition.start("examples/config/example-ignite.xml")) {
            try {
                ...
                DecisionTreeNode mdl = trainer.fit(
                    ignite,
                    dataCache,
                    FeatureLabelExtractorWrapper.wrap(imputingPreprocessor, CompositionUtils.asLabelExtractor(vectorizer)) //TODO: IGNITE-11581
                );
                ...
                double accuracy = Evaluator.evaluate(
                    dataCache,
                    mdl,
                    FeatureLabelExtractorWrapper.wrap(imputingPreprocessor, CompositionUtils.asLabelExtractor(vectorizer)),
                    new Accuracy<>()
                );
